...
start_app_session();
// Token CSRF para o formulário público de sugestões
$csrf_token = get_csrf_token();
// =====================================================================

// DETECTA SE A REQUISIÇÃO É AJAX (CHAVE PARA O LIVE SEARCH)
// Só retorna fragmento quando for requisição assíncrona real.
// Em algumas hospedagens/proxies, o header X-Requested-With pode não chegar.
...
$is_fetch_mode = in_array($sec_fetch_mode, ['cors', 'same-origin'], true);
$is_ajax = isset($_REQUEST['ajax']) && $_REQUEST['ajax'] == 1 && ($is_xhr || $is_fetch_mode);
...
$setores_internos = array_filter($lista_setores, function($s) {
    return $s !== 'externos' && $s !== 'centro_administrativo' && $s !== 'emails';
});
...
if (!$is_ajax):
    $css_version = @filemtime(__DIR__ . '/assets/css/styles.css') ?: time();
    $js_version = @filemtime(__DIR__ . '/assets/js/main.js') ?: time();
    $mascote_version = @filemtime(__DIR__ . '/mascote.js') ?: time();
    $mascote_css_version = @filemtime(__DIR__ . '/mascote.css') ?: time();
    $tour_version = @filemtime(__DIR__ . '/tour.js') ?: time();
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    ...
    <link rel="stylesheet" href="<?= BASE_PATH ?>/assets/css/styles.css?v=<?= $css_version ?>">
    <link rel="stylesheet" href="<?= BASE_PATH ?>/mascote.css?v=<?= $mascote_css_version ?>">
</head>
<body>
    <div class="admin-corner">
        <?php if (!empty($_SESSION['is_admin'])): ?>
            <a href="./admin.php" class="admin-link">Painel</a>
            <a href="./logout.php" class="admin-link admin-link-sair">Sair</a>
        <?php else: ?>
    ...
    <div class="container">
        <div class="header-container">
            ...
            <div class="header-actions">
                ...
                <button type="button" class="btn-sugestao" id="abrir-sugestao">💡 Sugestões</button>
            </div>
        </div>
...
if (!$is_ajax):
?>
    <!-- Modal: Enviar sugestão de melhoria -->
    <div id="modal-sugestao" class="sugestao-overlay" aria-hidden="true">
        <div class="sugestao-modal" role="dialog" aria-modal="true" aria-labelledby="sugestao-titulo">
            <div class="sugestao-header">
                <h2 id="sugestao-titulo">💡 Enviar sugestão de melhoria</h2>
                <button type="button" class="sugestao-close" id="fechar-sugestao" aria-label="Fechar">&times;</button>
            </div>
            <form id="form-sugestao">
                <p class="sugestao-intro">Encontrou algo que pode melhorar? Conte pra gente. Sua sugestão vai direto para a equipe.</p>
                <div class="sugestao-field">
                    <label for="sugestao-nome">Seu nome <span class="sugestao-opcional">(opcional)</span></label>
                    <input type="text" id="sugestao-nome" name="nome" maxlength="120" placeholder="Ex: Maria Silva">
                </div>
                <div class="sugestao-field">
                    <label for="sugestao-mensagem">Sua sugestão</label>
                    <textarea id="sugestao-mensagem" name="mensagem" rows="5" maxlength="2000" required placeholder="Descreva sua ideia ou o que poderia melhorar..."></textarea>
                </div>
                <div class="sugestao-feedback" id="sugestao-feedback" role="alert"></div>
                <div class="sugestao-actions">
                    <button type="button" class="sugestao-btn-cancelar" id="cancelar-sugestao">Cancelar</button>
                    <button type="submit" class="sugestao-btn-enviar" id="enviar-sugestao">Enviar sugestão</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        window.csrfToken = '<?= h($csrf_token) ?>';
    </script>
    <script src="<?= BASE_PATH ?>/assets/js/main.js?v=<?= $js_version ?>"></script>
    <script src="<?= BASE_PATH ?>/mascote.js?v=<?= $mascote_version ?>" defer></script>
    <script src="<?= BASE_PATH ?>/tour.js?v=<?= $tour_version ?>" defer></script>
</body>
</html>
